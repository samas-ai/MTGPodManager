...
/**
 * Resize to `count` seats, preserving the life of the seats that remain (unlike
 * initSeats, which resets everything). Used to auto-grow the grid as players
 * join a match without wiping totals mid-setup.
 */
export function resizeSeats(seats: Seat[], count: SeatCount): Seat[] {
  return Array.from(
    { length: count },
    (_, i) => seats[i] ?? { id: i + 1, life: STARTING_LIFE },
  );
}
